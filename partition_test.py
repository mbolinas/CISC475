...
#TODO: Generalize graph to work with the shortest path matrix provided by the other algorithms and to work with a graph of arbitrary size
...
def subgraph_generation():
    ...
    #Finds the tow nodes with the largest shortest path between them and creates new graphs starting from those nodes
    max_distance = 0;
    start_points = []
    for i in range (0, 9):
        for j in range (0, 9):
...
def next_node(g1, g2, node_list1, node_list2):
    new1 = None
    dist1 = None
    #finds next closest node to the graph G1
    for i in range(0, 9):
        for j in node_list1:
            if (dist1 > matrix[j-1][i][0] or dist1==None) and j-1 != i and g1.has_node(i+1)==False and G.has_node(i+1)==True:
                ...
                connecting = j
    #if the new node exists and does not already exist in G1, adds the node and the edge to G1
    if new1 != None:
        g1.add_node(new1)
        g1.add_edge(j, new1)

    #Performs the same task as the loop above, but on G2    
    new2 = None
    dist2 = None
    for i in range(0, 9):
        for j in node_list2:
    ...
    #If the next node to add is in both G1 and G2 then it is added tothe shared list
    if new1 == new2:
        shared.append(new1)
        G.remove_node(new1)
        return None

    #removes the nodes from original graph G
    if new1 != None:
        G.remove_node(new1)
